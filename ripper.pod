...
The I<rodeo2> Python script I<rodeo_main.py> produces and output file
named F<main_co_occur.csv>. An example of this is shown below.
...
F<ride_sp.pl> reads the F<main_co_occur.csv> file produced by
F<rodeo2> and

=over 2

=item 1. Determines the genbank accession to fetch based on the
value in column 3.

=item 2. Determines the coordinates of the tailoring enzyme by reading
...
variable I<$flankLen>. This smaller genbank file, hereafter referred
to as the I<sub-genbank> file, is centered around the centre of the
tailoring enzyme.

A specially built version of I<prodigal> renamed I<prodigal-short> is
then used to find genes in the sub-genbank file. I<prodigal-short> is
...
For all the genes found by I<prodigal-short> the following is done.
...
included in the list to be output and also saved in a I<Sqlite3>
table.

=head2 Example Bash script implementing the full analysis pipeline

The starting point is a query file containing Genpept accessions for
tailoring enzymes. In the example below this file is named F<in.list>.
...
contain F<local.conf> and the Bash script listed below.

Example of Bash script to run F<rodeo_main.py> followed by
F<ride_sp.pl> and some other scripts to organise the output and output
files.
...
=head2 Example of F<local.conf>

F<local.conf> is a two column (space delimited) text file which is
read by F<ride_sp.pl> and the following scripts in the pipeline.
...
=for comment {{{ Building prodigal-short

=head2 Building I<prodigal-short>

The following changes (shown as the output of the C<git diff> command)
were made to I<prodigal> source files before building
I<prodigal-short> according to instructions provided with the
I<prodigal> source download.
...
=head1 Other supporting Perl scripts

=head2 pfam_sqlite.pl

F<pfam_sqlite.pl> takes all the proteins in a specified table in a
...
=head2 mergeRidePfam.pl

F<mergeRidePfam.pl> merges the information contained in the two sqlite3
...
=head2 collectFiles.pl

F<collectFiles.pl> copies files from a specified directory (and
...
The options shown above are the defaults. C<-outdir> may be specified
in F<local.conf> as I<rodeohtmldir>. Value in the configuration file
takes precedence.
